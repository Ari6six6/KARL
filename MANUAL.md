...
| tach climbs for minutes, `no tokens yet` hint appears | server overloaded, still loading, model spilled to **CPU** (VRAM too tight) — or the box's GPU is **dead** | `karl doctor` names which; it checks `nvidia-smi` on the box, the offloaded-layer count, and whether the model actually sits in VRAM. Dead GPU → destroy the instance and rent another; spill → smaller model (`karl gpu model qwen`) |
